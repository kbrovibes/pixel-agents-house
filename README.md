...
1. **iPad / iPhone**: open `http://pixelagents.local/ca.crt` in Safari. Safari asks to download a configuration profile; tap Allow. Then Settings → *Profile Downloaded* (near the top; on older iOS it is under General → VPN & Device Management) → Install. Finally Settings → General → About → Certificate Trust Settings (this entry only exists once a certificate profile is installed) → enable full trust for *Pixel Agents House Local CA*. Open `https://pixelagents.local`, Share → Add to Home Screen.
   If Safari saved the file to Downloads instead of asking, open the Files app, tap `ca.crt`, and continue from *Profile Downloaded*. AirDrop works too: on the Mac run `open ~/.pixelagents/tls`, AirDrop `ca.crt` to the iPad, and the same prompt appears.
