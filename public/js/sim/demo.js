...
export function createDemoWatcher(opts = {}) {
  ...
  function newAgent(longIdle = false) {
    ...
    const a = {
      ...
      startedAt: now - Math.floor(rng() * 3600000), lastActivityAt: now, contextTokens: 20000 + Math.floor(rng() * 160000), outputTokens: Math.floor(rng() * 40000),
      tmux: rng() < 0.7 ? { session: pick(['AGENTS', 'BADDY', 'DOODLE', 'MONEY', 'STUDY', 'OPS'], rng), window: '2.1' } : null,
